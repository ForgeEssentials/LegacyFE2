...
import com.forgeessentials.api.areas.Point;
import com.forgeessentials.api.areas.WarpPoint;
...
// If something is commented out, means it hasn't been ported to FE2 yet. Either wait for me to port it or do it yourself. -luacs1998
public class FunctionHelper {
	public static Pattern groupRegex = Pattern
	...
	public static String getZoneWorldString(World world) {
		return "WORLD_" + world.provider.getDimensionName().replace(' ', '_')
				+ "_" + world.provider.dimensionId;
	}
	...
	/*
	 * /** does NOT check if its a valid BlockID and stuff.. this may be used
	 * for items.
	 * 
	 ...
	 * 
	 * public static int[] parseIdAndMetaFromString(String msg, boolean
	 * blocksOnly) throws NumberFormatException { int ID; int meta = -1;
	 * 
	 * // perhaps the ID:Meta format if (msg.contains(":")) { String[] pair =
	 * msg.split(":", 2);
	 * 
	 * try { ID = Integer.parseInt(pair[0]); } catch (NumberFormatException e) {
	 * ID = getItemIDFromName(pair[0], blocksOnly); }
	 * 
	 * try { meta = Integer.parseInt(pair[1]); } catch (NumberFormatException e)
	 * { throw new NumberFormatException(pair[1] +
	 * " param was not recognized as number. Please try again."); } } else { try
	 * { ID = Integer.parseInt(msg); meta = -1; } catch (NumberFormatException
	 * e) { ID = getItemIDFromName(msg, blocksOnly); } }
	 * 
	 * // try checking if its just an ID
	 * 
	 * return new int[] { ID, meta }; }
	 * 
	 * public static int getItemIDFromName(String name, boolean blockOnly) { if
	 * (blockOnly) { Block block =
	 * FriendlyItemList.instance().getBlockForName(name); if (block == null)
	 * return 0; else return block.blockID; } else { Item item =
	 * FriendlyItemList.instance().getItemForName(name); if (item == null)
	 * return 0; else return item.itemID; } }
	 */
	 ...
	 * @return
	 */
	public static String replaceAllIgnoreCase(String text, String search,
			String replacement) {
	...
	public static String format(String format) {
		...
		// replace colors
		format = replaceAllIgnoreCase(format, "%red",
				EnumChatFormatting.RED.toString());
		format = replaceAllIgnoreCase(format, "%yellow",
				EnumChatFormatting.YELLOW.toString());
		format = replaceAllIgnoreCase(format, "%black",
				EnumChatFormatting.BLACK.toString());
		format = replaceAllIgnoreCase(format, "%darkblue",
				EnumChatFormatting.DARK_BLUE.toString());
		format = replaceAllIgnoreCase(format, "%darkgreen",
				EnumChatFormatting.DARK_GREEN.toString());
		format = replaceAllIgnoreCase(format, "%darkaqua",
				EnumChatFormatting.DARK_AQUA.toString());
		format = replaceAllIgnoreCase(format, "%darkred",
				EnumChatFormatting.DARK_RED.toString());
		format = replaceAllIgnoreCase(format, "%purple",
				EnumChatFormatting.LIGHT_PURPLE.toString());
		format = replaceAllIgnoreCase(format, "%gold",
				EnumChatFormatting.GOLD.toString());
		format = replaceAllIgnoreCase(format, "%grey",
				EnumChatFormatting.GRAY.toString());
		format = replaceAllIgnoreCase(format, "%darkgrey",
				EnumChatFormatting.DARK_GRAY.toString());
		format = replaceAllIgnoreCase(format, "%indigo",
				EnumChatFormatting.DARK_PURPLE.toString());
		format = replaceAllIgnoreCase(format, "%green",
				EnumChatFormatting.GREEN.toString());
		format = replaceAllIgnoreCase(format, "%aqua",
				EnumChatFormatting.AQUA.toString());
		format = replaceAllIgnoreCase(format, "%pink",
				EnumChatFormatting.RED.toString());
		format = replaceAllIgnoreCase(format, "%white",
				EnumChatFormatting.WHITE.toString());

		// replace MC formating
		format = replaceAllIgnoreCase(format, "%random",
				EnumChatFormatting.OBFUSCATED.toString());
		format = replaceAllIgnoreCase(format, "%bold",
				EnumChatFormatting.BOLD.toString());
		format = replaceAllIgnoreCase(format, "%strike",
				EnumChatFormatting.STRIKETHROUGH.toString());
		format = replaceAllIgnoreCase(format, "%underline",
				EnumChatFormatting.UNDERLINE.toString());
		format = replaceAllIgnoreCase(format, "%italics",
				EnumChatFormatting.ITALIC.toString());
		format = replaceAllIgnoreCase(format, "%reset",
	...
	public static void setPlayer(EntityPlayerMP player, WarpPoint p) {
		if (player.dimension != p.dim) {
			MinecraftServer.getServer().getConfigurationManager()
					.transferPlayerToDimension(player, p.dim);
		}
		player.playerNetServerHandler.setPlayerLocation(p.xd, p.yd, p.zd,
				p.yaw, p.pitch);
	...
	/*
	 * public static String getGroupRankString(String username) { Matcher match
	 * = groupRegex.matcher(SystemConfig.groupRankFormat);
	 * ArrayList<TreeSet<Group>> list = getGroupsList(match, username);
	 * 
	 * String end = "";
	 * 
	 * StringBuilder temp = new StringBuilder(); for (TreeSet<Group> set : list)
	 * { for (Group g : set) { if (temp.length() != 0) { temp.append("&r"); }
	 * 
	 * temp.append(g.name); }
	 * 
	 * end = match.replaceFirst(temp.toString()); temp = new StringBuilder(); }
	 * 
	 * return end; }
	 * 
	 * public static String getGroupPrefixString(String username) { Matcher
	 * match = groupRegex.matcher(SystemConfig.groupPrefixFormat);
	 * 
	 * ArrayList<TreeSet<Group>> list = getGroupsList(match, username);
	 * 
	 * String end = "";
	 * 
	 * StringBuilder temp = new StringBuilder(); for (TreeSet<Group> set : list)
	 * { for (Group g : set) { if (g.prefix.trim().isEmpty()) { continue; }
	 * 
	 * if (temp.length() == 0) { temp.append(g.prefix); } else { temp.insert(0,
	 * g.prefix + "&r"); } }
	 * 
	 * end = match.replaceFirst(temp.toString()); temp = new StringBuilder(); }
	 * 
	 * return end; }
	 * 
	 * /* public static String getGroupSuffixString(String username) { Matcher
	 * match = groupRegex.matcher(SystemConfig.groupSuffixFormat);
	 * 
	 * ArrayList<TreeSet<Group>> list = getGroupsList(match, username);
	 * 
	 * String end = "";
	 * 
	 * StringBuilder temp = new StringBuilder(); for (TreeSet<Group> set : list)
	 * { for (Group g : set) { if (g.suffix.trim().isEmpty()) { continue; }
	 * 
	 ...
	 * 
	 * 
	 * private static ArrayList<TreeSet<Group>> getGroupsList(Matcher match,
	 * String username) { ArrayList<TreeSet<Group>> list = new
	 * ArrayList<TreeSet<Group>>();
	 * 
	 * String whole; String[] p; TreeSet<Group> set; while (match.find()) {
	 * whole = match.group(); whole = whole.replaceAll("\\{",
	 * "").replaceAll("\\}", ""); p = whole.split("\\<\\:\\>", 2); if
	 * (p[0].equalsIgnoreCase("...")) { p[0] = null; } if
	 * (p[1].equalsIgnoreCase("...")) { p[1] = null; }
	 * 
	 * set = SqlHelper.getGroupsForChat(p[0], p[1], username); if (set != null)
	 * { list.add(set); } }
	 * 
	 * list = removeDuplicates(list); return list; }
	 */

	private static ArrayList<TreeSet<Group>> removeDuplicates(
			ArrayList<TreeSet<Group>> list) {
	...
	/*
	 * public static String getFormattedPlayersOnline() { StringBuilder sb = new
	 * StringBuilder(); for (Object fakePlayer :
	 * MinecraftServer.getServer().getConfigurationManager().playerEntityList) {
	 * EntityPlayer player = (EntityPlayer) fakePlayer; String name =
	 * player.username; if
	 * (player.getEntityData().getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG
	 * ).hasKey("nickname")) { name =
	 * player.getEntityData().getCompoundTag(EntityPlayer
	 * .PERSISTED_NBT_TAG).getString("nickname"); } name =
	 * getGroupRankString(player.username) + ":" + name; sb.append(name + ", ");
	...
	public static void saveBookToFile(ItemStack book, File savefolder) {
		...
		if (book != null) {
			if (book.hasTagCompound()) {
				if (book.getTagCompound().hasKey("title")
						&& book.getTagCompound().hasKey("pages")) {
					filename = book.getTagCompound().getString("title")
							+ ".txt";
	...
	public static void getBookFromFile(EntityPlayer player, File file) {
		...
		if (file.isFile()) {
			if (file.getName().contains(".txt")) {
				...
				try {
					lines.add(EnumChatFormatting.GREEN + "START"
					...
					stream.close();
					lines.add("");
					lines.add(EnumChatFormatting.RED + "END"
							+ EnumChatFormatting.BLACK);

				} catch (Exception e) {
					OutputHandler.felog.warning("Error reading script: "
							+ file.getName());
				}
				int part = 0;
				...
				while (lines.size() != 0) {
					...
					map.put(EnumChatFormatting.GOLD + " File: "
							+ EnumChatFormatting.GRAY + filename
		...
		if (file.isFile()) {
			if (file.getName().contains(".txt")) {
				...
				try {
					...
					OutputHandler.felog.warning("Error reading book: "
							+ file.getName());
				}
				while (lines.size() != 0) {
					String temp = "";
	...
	public static void getBookFromFileUnformatted(EntityPlayer player,
