...
public class ForgeConfigDataDriver extends TextDataDriver {
	...
	protected boolean saveData(ClassContainer type, TypeData objectData) {
		...
		for (Entry<String, Object> entry : objectData.getAllFields()) {
			writeFieldToProperty(cfg, type.getFileSafeName(), entry.getKey(),
					entry.getValue());
		}
	...
	protected TypeData loadData(ClassContainer type, String uniqueKey) {
		...
		readClassFromProperty(cfg, cfg.getCategory(type.getFileSafeName()),
				data, info);
	...
	private void writeFieldToProperty(Configuration cfg, String category,
		...
		if (type.equals(Integer.class)) {
			...
			for (Entry<String, Object> entry : data.getAllFields()) {
				writeFieldToProperty(cfg, newcat, entry.getKey(),
						entry.getValue());
			}
		} else
			throw new IllegalArgumentException("Cannot save object type. "
					+ obj.getClass() + "  instance: " + obj);
	}
	...
	private Object readFieldFromProperty(Configuration cfg, String category,
		...
		if (type.equals(byte.class))
			...
			double[] array = cfg.get(category, name, new double[] {})
					.getDoubleList();
	...
	private void readClassFromProperty(Configuration cfg, ConfigCategory cat,
			TypeData data, ITypeInfo<?> info) {
		...
		if (cat != null) {
			...
			for (Property prop : cat.getValues().values()) {
				...
				value = readFieldFromProperty(cfg, cat.getQualifiedName(),
			...
			for (String childName : cfg.getCategoryNames()) {
				...
				if (child.isChild() && child.parent == cat) // intentional use
															// of ==
				{
					name = child.getQualifiedName().replace(
							cat.getQualifiedName() + ".", "");
					newInfo = info.getInfoForField(name);
					newData = DataStorageManager.getDataForType(newInfo
							.getType());
