...
import com.forgeessentials.util.data.api.IDataDriver;
import com.forgeessentials.util.data.api.IDataDriver.EnumDriverType;
import com.forgeessentials.util.data.api.IStorageManager;
...
import com.forgeessentials.util.data.types.TypeInfoArray;
import com.forgeessentials.util.data.types.TypeInfoList;
import com.forgeessentials.util.data.types.TypeInfoMap;
import com.forgeessentials.util.data.types.TypeInfoSerialize;
import com.forgeessentials.util.data.types.TypeInfoSet;
import com.forgeessentials.util.data.types.TypeInfoStandard;
...
public class StorageManager implements IStorageManager {
	...
	private ConcurrentHashMap<EnumDriverType, String> typeChosens; // the
																	// defaults...
	private ConcurrentHashMap<String, Class<? extends AbstractDataDriver>> classMap; // registered
																						// ones...
	private ConcurrentHashMap<String, AbstractDataDriver> instanceMap; // instantiated
																		// ones
	...
	public StorageManager(Configuration config) {
		...
		prop.comment = "Specifies the variety of data storage FE will use. Options: "
				+ FunctionHelper.niceJoin(EnumDriverType.values());
	...
	public void setupManager() {
		...
		for (Entry<String, AbstractDataDriver> entry : instanceMap.entrySet()) {
			try {
				...
				entry.getValue().parseConfigs(
						config,
						"Data." + entry.getValue().getType() + "."
								+ entry.getValue().getName());
			} catch (Exception e) {
				OutputHandler.felog.info("Problem loading DataDriver "
						+ entry.getKey());
				OutputHandler.felog
						.info("ForgeEssentials will not be able to save any data through this driver");
				e.printStackTrace();
			}
		}
	}
	...
	public void registerDriver(String name,
			Class<? extends AbstractDataDriver> c) {
		try {
	...
	public void registerSaveableClass(ClassContainer type) {
		...
		if (type.isArray()
				&& !type.getType().getComponentType().isPrimitive()
				&& !String.class.isAssignableFrom(type.getType()
						.getComponentType())) {
	...
	@Override
	public void registerSaveableClass(Class<? extends ITypeInfo> infoType,
			ClassContainer type) {
			...
			try {
				...
				if (con == null)
					throw new IllegalArgumentException(
							infoType.getCanonicalName()
									+ " must have useable constructors! See the ITypeInfo documentation!");
				...
				OutputHandler.felog
						.severe(infoType.getCanonicalName()
								+ " must have useable constructors! See the ITypeInfo documentation!");
	...
	public ITypeInfo getInfoForType(ClassContainer type) {
		...
		while (!isClassRegisterred(tempType)) {
			if (tempType == null) {
				...
				tempType = new ClassContainer(tempType.getType()
						.getSuperclass(), type.getParameters());
			} else if (tempType.getType().getSuperclass() == null) {
				tempType = null;
			}
		}
	...
	public TypeData getDataForObject(ClassContainer container, Object obj) {
		...
		for (Entry<String, Object> e : data.getAllFields()) {
			if (e.getValue() != null && !(e.getValue() instanceof TypeData)
					&& StorageManager.isTypeComplex(e.getValue().getClass())) {
				tempInfo = info.getInfoForField(e.getKey());
				data.putField(
						e.getKey(),
						DataStorageManager.getDataForObject(
								tempInfo.getTypeOfField(e.getKey()),
								e.getValue()));
			}
		}
	...
	public static boolean isTypeComplex(Class type) {
		...
		if (type.isPrimitive() || type.equals(Integer.class)
			...
			return !(type.getComponentType().isPrimitive()
					|| type.getComponentType().equals(Integer.class)
					|| type.getComponentType().equals(Float.class)
					|| type.getComponentType().equals(Double.class)
					|| type.getComponentType().equals(Boolean.class)
					|| type.getComponentType().equals(String.class) || type
						.equals(Byte.class));
