...
public class DBConnector {
	 ...
	 */
	public DBConnector(String name, DBConnector fallback, EnumDBType dType,
			String dbDefault, String dbFileDefault, boolean useFallback) {
		this.name = name;
	...
	public void write(Configuration config, String cat) {
		config.get(cat, "chosenType", dType.toString(),
				" valid types: " + FunctionHelper.niceJoin(EnumDBType.values()))
				.set(type.toString());
		...
		for (EnumDBType type : EnumDBType.values()) {
			...
			if (type.isRemote) {
				config.get(newcat, "host", "localhost").set(
						props.get("host").getString());
				config.get(newcat, "port", 3360).set(
						props.get("port").getString());
				config.get(newcat, "database", dbDefault).set(
						props.get("database").getString());
				config.get(newcat, "user", "FEUSER").set(
						props.get("user").getString());
				config.get(newcat, "pass", "password").set(
						props.get("pass").getString());
			} else {
				config.get(newcat, "database", dbFileDefault,
						"this may be a file path as well.").set(
						props.get("database").getString());
			}
	...
	public void loadOrGenerate(Configuration config, String cat) {
		try {
			tempType = type = EnumDBType.valueOf(config.get(cat, "chosenType",
					dType.toString()).getString());
			if (fallback != null) {
				useParent = config.get(cat, "checkParent", false).getBoolean(
						false);
			}
		} catch (Exception e) {
		...
		for (EnumDBType type : EnumDBType.values()) {
			...
			if (type.isRemote) {
				...
				props.put("database", config.get(newcat, "database",
						dbFileDefault, "this may be a file path as well."));
			}

		}

		config.get(cat, "chosenType", type.toString(), " valid types: "
				+ FunctionHelper.niceJoin(EnumDBType.values()));
		config.get(
				cat,
				"checkParent",
				useParent,
	...
	public Connection getChosenConnection() {
		...
		try {
			...
			if (type.isRemote) {
				...
				if (useParent) {
					...
					if (con != null)
						...
						OutputHandler.felog.warning("[FE+SQL] " + name
								+ " Parent check failed, going to in-house.");
					}
				}
			...
			OutputHandler.felog.log(Level.WARNING, "[FE+SQL] " + name
					+ " In-House check failed, going to default.", e);
		}
		...
		try {
			...
			if (dType.isRemote) {
				...
				dType.loadClass();
				String connect = dType
						.getConnectionString(host, port, database);
			...
			OutputHandler.felog.severe("[FE+SQL] " + name
					+ " CATASTROPHIC DATABASE CONNECTION FAILIURE!!!");
	 ...
	 */
	private Connection getSpecificConnection(EnumDBType type)
			throws IllegalArgumentException {
		if (!type.isRemote)
			throw new IllegalArgumentException("Non remote type " + type
					+ " is asking for parent config!");
