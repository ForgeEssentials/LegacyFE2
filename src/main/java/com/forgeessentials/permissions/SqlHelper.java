...
import com.forgeessentials.permissions.Permission.PermissionHolder;
import com.forgeessentials.permissions.PermissionProp.PermissionPropHolder;
...
public class SqlHelper
{
	...
	public SqlHelper(ConfigPermissions config)
	{
		...
		prepareStatements(db, config.connector.getActiveType());
	}
	 ...
	 */
	private void prepareStatements(Connection db, EnumDBType type)
	{
		try
		{
			...
			query = new StringBuilder("INSERT INTO ").append(TABLE_PERMISSION).append(" (")
	 ...
	 */
	protected static synchronized PermResult getPermissionResult(String target, boolean isGroup, PermissionsChecker perm, String zone, boolean checkForward)
	{
		try
		{
			int tID;
			...
			while (perm != null)
			{
				...
				{
					perm = new PermissionsChecker(perm.getAllParent());
				}
			}
	...
	protected static synchronized boolean setPermission(String target, boolean isGroup, Permission perm, String zone)
	{
		try
		{
			...
			PreparedStatement use = getInstance().statementGetPermission;
