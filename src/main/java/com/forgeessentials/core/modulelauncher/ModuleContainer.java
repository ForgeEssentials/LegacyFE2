...
public class ModuleContainer implements Comparable {
	...
	// methods..
	private String preinit, init, postinit, serverinit, serverpostinit,
			serverstopping, serverstopped, reload;
	...
	public ModuleContainer(ASMData data) {
		...
		try {
			...
			OutputHandler.felog.info("Error trying to load "
					+ data.getClassName() + " as a FEModule!");
			e.printStackTrace();
		...
		if (!c.isAnnotationPresent(FEComponent.class))
			throw new IllegalArgumentException(c.getName()
					+ " doesn't have the @FEModule annotation!");
		...
		if (serverOnly
			...
			OutputHandler.felog
					.warning("The module "
							+ name
							+ " has been coded to work on the dedicated server only. It will not be loaded.");
		}
			...
			if (m.isAnnotationPresent(FEModuleEventHandler.class)) {
				if (preinit != null)
					throw new RuntimeException(
							"Only one class may be marked as FEModuleEventHandler");
		...
		for (Field f : c.getDeclaredFields()) {
			if (f.isAnnotationPresent(Instance.class)) {
				if (instance != null)
					throw new RuntimeException(
							"Only one field may be marked as Instance");
				f.setAccessible(true);
				instance = f.getName();
			} else if (f.isAnnotationPresent(Container.class)) {
				if (container != null)
					throw new RuntimeException(
							"Only one field may be marked as Container");
				if (f.getType().equals(ModuleContainer.class))
					throw new RuntimeException(
							"This field must have the type ModuleContainer!");
				f.setAccessible(true);
				container = f.getName();
			} else if (f.isAnnotationPresent(ModuleDir.class)) {
				if (moduleDir != null)
					throw new RuntimeException(
							"Only one field may be marked as ModuleDir");
				if (!File.class.isAssignableFrom(f.getType()))
					throw new RuntimeException(
							"This field must be the type File!");
				f.setAccessible(true);
				moduleDir = f.getName();
			}
		}
	...
	protected void createAndPopulate() {
		...
		try {
			...
			OutputHandler.felog
					.warning(name
							+ " could not be instantiated. FE will not load this module.");
			e.printStackTrace();
			isLoadable = false;
			return;
		}
		...
		try {
			configObj = configClass.getConstructor(File.class).newInstance(
					new File(ForgeEssentials.FEDIR, name + "/config.cfg"));
			...
			OutputHandler.felog
					.info("Error Instantiating or populating config for "
							+ name);
			Throwables.propagate(e);
		}
	}
	...
	public void runPreInit(FMLPreInitializationEvent fmlEvent, CallableMap map) {
		...
		try {
			...
			Method m = c.getDeclaredMethod(preinit,
					new Class[] { FMLPreInitializationEvent.class });
			m.invoke(module, fmlEvent);
		} catch (Throwable e) {
			OutputHandler.felog
					.severe("Error while invoking preInit event for " + name);
			Throwables.propagate(e);
		}
	}
	...
	public void runInit(FMLInitializationEvent fmlEvent) {
		...
		try {
			...
			OutputHandler.felog.info("Error while invoking Init event for "
					+ name);
			Throwables.propagate(e);
		}
	}
	...
	public void runPostInit(FMLPostInitializationEvent fmlEvent) {
		...
		try {
			Class c = Class.forName(className);
			Method m = c.getDeclaredMethod(postinit,
					new Class[] { FMLPostInitializationEvent.class });
			m.invoke(module, fmlEvent);
		} catch (Throwable e) {
			OutputHandler.felog.info("Error while invoking PostInit event for "
					+ name);
			Throwables.propagate(e);
		}
	}
	...
	public void runServerInit(FMLServerStartingEvent fmlEvent) {
		...
		try {
			Class c = Class.forName(className);
			Method m = c.getDeclaredMethod(serverinit,
					new Class[] { FMLServerStartingEvent.class });
			m.invoke(module, fmlEvent);
		} catch (Throwable e) {
			OutputHandler.felog
					.info("Error while invoking ServerInit event for " + name);
			Throwables.propagate(e);
		}
	}
	...
	public void runServerPostInit(FMLServerStartedEvent fmlEvent) {
		...
		try {
			Class c = Class.forName(className);
			Method m = c.getDeclaredMethod(serverpostinit,
					new Class[] { FMLServerStartedEvent.class });
			m.invoke(module, fmlEvent);
		} catch (Throwable e) {
			OutputHandler.felog
					.info("Error while invoking ServerPostInit event for "
							+ name);
			Throwables.propagate(e);
		}
	}
	...
	public void runServerStop(FMLServerStoppingEvent fmlEvent) {
		...
		try {
			Class c = Class.forName(className);
			Method m = c.getDeclaredMethod(serverstopping,
					new Class[] { FMLServerStoppingEvent.class });
			m.invoke(module, fmlEvent);
		} catch (Throwable e) {
			OutputHandler.felog
					.info("Error while invoking ServerStop event for " + name);
			Throwables.propagate(e);
		}
	...
	public void runServerStopped(FMLServerStoppedEvent fmlEvent) {
		...
		try {
			Class c = Class.forName(className);
			Method m = c.getDeclaredMethod(serverstopping,
					new Class[] { FMLServerStoppedEvent.class });
			m.invoke(module, fmlEvent);
		} catch (Throwable e) {
			OutputHandler.felog
					.info("Error while invoking ServerStopped event for "
							+ name);
			Throwables.propagate(e);
		}
	...
	public boolean equals(Object o) {
		...
		return isCore == c.isCore && name.equals(c.name)
