		...
		for (ASMData asm : data) {
			...
			if (temp.isLoadable) {
				if (containerMap.containsKey(temp.name)) {
					...
					if (temp.doesOverride
							&& other.mod == ForgeEssentials.instance) {
						containerMap.put(temp.name, temp);
					} else if (temp.mod == ForgeEssentials.instance
							&& other.doesOverride) {
						continue;
					} else
						throw new RuntimeException("{FE-Module-Launcher} "
								+ temp.name + " is conflicting with "
								+ other.name);
				} else {
					containerMap.put(temp.name, temp);
				}
